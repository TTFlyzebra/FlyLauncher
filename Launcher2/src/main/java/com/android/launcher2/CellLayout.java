...
public class CellLayout extends ViewGroup {
        ...
        for (int i = 0; i < mDragOutlineAnims.length; i++) {
            ...
            anim.getAnimator().addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    BigDecimal bigDecAnimaValue = BigDecimal.valueOf((float)(((ValueAnimator)animation).getAnimatedValue()));
                    if (bigDecAnimaValue.compareTo(BigDecimal.ZERO) == 0) {
                        anim.setTag(null);
                    }
                }
            });
    ...
    public void scaleRect(Rect r, float scale) {
        BigDecimal bigDecScale = BigDecimal.valueOf(scale);
        if (bigDecScale.compareTo(BigDecimal.valueOf(1.0f)) != 0) {
    ...
    public void setBackgroundAlphaMultiplier(float multiplier) {
        BigDecimal bigDecCurAlphaMulti = BigDecimal.valueOf(mBackgroundAlphaMultiplier);
        if (bigDecCurAlphaMulti.compareTo(BigDecimal.valueOf(multiplier)) != 0) {
    ...
    public void setBackgroundAlpha(float alpha) {
        BigDecimal bigDecCurAlpha = BigDecimal.valueOf(mBackgroundAlpha);
        if (bigDecCurAlpha.compareTo(BigDecimal.valueOf(alpha)) != 0) {
    ...
    int[] findNearestArea(int pixelX, int pixelY, int minSpanX, int minSpanY, int spanX, int spanY,
        ...
        for (int y = 0; y < countY - (minSpanY - 1); y++) {
            ...
            for (int x = 0; x < countX - (minSpanX - 1); x++) {
                ...
                double distance = Math.sqrt(Math.pow((double)(cellXY[0] - pixelX), 2)
                        + Math.pow((double)(cellXY[1] - pixelY), 2));
        ...
        BigDecimal bigDecBestDistance = BigDecimal.valueOf(bestDistance);
        if (bigDecBestDistance.compareTo(BigDecimal.valueOf(Double.MAX_VALUE)) == 0) {
            bestXY[0] = -1;
            bestXY[1] = -1;
        }
    ...
    private int[] findNearestArea(int cellX, int cellY, int spanX, int spanY, int[] direction,
        ...
        for (int y = 0; y < countY - (spanY - 1); y++) {
            ...
            for (int x = 0; x < countX - (spanX - 1); x++) {
                ...
                float distance = (float)
                        Math.sqrt((double)((x - cellX) * (x - cellX)) + (double)((y - cellY) * (y - cellY)));
        ...
        // Return -1, -1 if no suitable location found
        BigDecimal bigDecBestDistance = BigDecimal.valueOf(bestDistance);
        if (bigDecBestDistance.compareTo(BigDecimal.valueOf(Float.MAX_VALUE)) == 0) {
            bestXY[0] = -1;
            bestXY[1] = -1;
        }
        return bestXY;
    ...
    private boolean addViewsToTempLocation(ArrayList<View> views, Rect rectOccupiedByPotentialDrop,
        ...
        if (boundingRect == null) {
            LauncherLog.w(TAG, "addViewsToTempLocation fail, no boudingrect ");
            return false;
        }
    ...
    class ReorderHintAnimation {
        ...
        void animate() {
            if (mShakeAnimators.containsKey(child)) {
                ...
                if ((BigDecimal.valueOf(finalDeltaX).compareTo(BigDecimal.ZERO) == 0)
                    && (BigDecimal.valueOf(finalDeltaY).compareTo(BigDecimal.ZERO) == 0)) {
