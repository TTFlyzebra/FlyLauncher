...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    public PagedView(Context context, AttributeSet attrs, int defStyle) {
        ...
        /// M: [ALPS01106267] If workspace_page_spacing is negative, set page spacing as AUTOMATIC_PAGE_SPACING.
        int pageSpacing = a.getDimensionPixelSize(R.styleable.PagedView_pageSpacing, 0);
        if (pageSpacing < 0) {
            setPageSpacing(AUTOMATIC_PAGE_SPACING);
        } else {
    ...
    public void scrollTo(int x, int y) {
        ...
        if (isXBeforeFirstPage) {
            ...
            if (isSupportCycleSlidingScreen()) {
                ...
                if (mAllowOverScroll) {
                    if (isRtl) {
                        overScroll((float)(x - mMaxScrollX));
                    } else {
                        overScroll(x);
                    }
                }
    ...
    protected void determineScrollingStart(MotionEvent ev) {
        // M: change touchSlpoScale from 1.0f to 0.5f
    ...
    protected void acceleratedOverScroll(float amount) {
        ...
        if (screenSize <= 0) {
            LauncherLog.e(TAG, "acceleratedOverScroll has invalid scrren width");
            return;
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_UP:
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                BigDecimal bigDecVelocityX = BigDecimal.valueOf(Math.signum(velocityX));
                BigDecimal bigDecDeltaX = BigDecimal.valueOf(Math.signum(deltaX));
                if (Math.abs(deltaX) > pageWidth * RETURN_TO_ORIGINAL_PAGE_THRESHOLD
                    && (bigDecVelocityX.compareTo(bigDecDeltaX) != 0)
                    && isFling) {
        ...
        if ((event.getSource() & InputDevice.SOURCE_CLASS_POINTER) != 0) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_SCROLL: {
                    ...
                    BigDecimal bigDecHscroll = BigDecimal.valueOf(hscroll);
                    BigDecimal bigDecVscroll = BigDecimal.valueOf(vscroll);
                    if ((bigDecHscroll.compareTo(BigDecimal.ZERO) != 0)
                        || (bigDecVscroll.compareTo(BigDecimal.ZERO) != 0)) {
    ...
    protected void loadAssociatedPages(int page, boolean immediateAndOnly) {
        if (mContentIsRefreshable) {
            ...
            if (page < count) {
                ...
                /// M: sync the first page if the current upper bound is over the last page when circle sliding is
                /// supported.
                ...
                /// M: sync the last page if the current lower bound is less than the first page when circle sliding is
                /// supported.
