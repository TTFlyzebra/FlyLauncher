...
public class LauncherModel extends BroadcastReceiver {
    ...
    private AlreadyInDesktopApp mAlreadyInDesktopApp = new AlreadyInDesktopApp();
    ...
    public class AlreadyInDesktopApp {
        ...
        private class ShortcutFlag {
            ...
            @Override
            public int hashCode() {
                return (packegName + componentName).hashCode();
            }
        }
    }
    ...
    private class LoaderTask implements Runnable {
                    ...
                    while (!mStopped && c.moveToNext()) {
                        try {
                            ...
                            case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
                                ...
                                if (itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION) {
                                    ...
                                    if (info != null) {
                                        ...
                                        if (info.cellX >= getCellCountX()
                                            || info.cellY >= getCellCountY()) {
                                            itemsToRemove.add(c.getLong(idIndex));
                                            if (DEBUG_LOADERS) {
                                                Log.e(TAG, "loadWorkspace, item type is application, cellx || celly error"
                                                    + ", package name: " + ShortcutInfo.getPackageName(intent)
                                                    + ", class name: " + info.componentName.getClassName()
                                                    + ", cellX: " + info.cellX + ", cellY: " + info.cellY);
                                            }
                                            continue;
                                        }
                                        
                                        mAlreadyInDesktopApp.addShortcutFlag(
                                                    ShortcutInfo.getPackageName(intent),
                                                    info.componentName.getClassName());
                                        if (DEBUG_LOADERS) {
                                            Log.d(TAG, "loadWorkspace, item type is application, "
                                                    + ", package name: " + ShortcutInfo.getPackageName(intent)
                                                    + ", class name: " + info.componentName.getClassName());
                                        }
                                    } else {
                                        Log.d(TAG, "loadWorkspace, item type is application, but cann't get application info, intent : " + intent);
                                    }
                                } else {
                                ...
                                if (info != null) {
                                    ...
                                    if (container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
                                        mAlreadyInDesktopApp.addScreen(info.screen);
                                    }
                                    ...
                                    case LauncherSettings.Favorites.CONTAINER_HOTSEAT:
                                        if (!sBgWorkspaceItems.contains(info)) {
                            ...
                            case LauncherSettings.Favorites.ITEM_TYPE_FOLDER:
                                ...
                                if (container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
                                    mAlreadyInDesktopApp.addScreen(folderInfo.screen);
                                }
                                ...
                                switch (container) {
                                    ...
                                    case LauncherSettings.Favorites.CONTAINER_HOTSEAT:
                                        ...
                                        } else if (LauncherLog.DEBUG) {
                                            LauncherLog.d(TAG, "loadWorkspace sBgWorkspaceItems already exsit, floderInfo = " + folderInfo);
                                        }
                                        break;
                                }
                            ...
                            case LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET:
                                ...
                                if (!isSafeMode && (provider == null || provider.provider == null ||
                                    ...
                                    mAlreadyInDesktopApp.addScreen(appWidgetInfo.screen);
        ...
        private void filterCurrentWorkspaceItems(int currentScreen,
            ...
            if (DEBUG_LOADERS) {
                Log.d(TAG, "filterCurrentWorkspaceItems enter, item cnt = "
                    + allWorkspaceItems.size());
            }
        ...
        private void loadAndBindAllNotInDesktopApps() {
            if (LauncherLog.DEBUG_LOADER) {
                LauncherLog.d(TAG, "loadAndBindAllNotInDesktopApps: mAllAppsLoaded =" + mAllAppsLoaded
        ...
        private void loadNotInDesktopAllAppsByBatch() {
            ...
            for (int p = 0; p < profileCount; p++) {
                ...
                while (i < N && !mStopped) {
                    ...
                    for (int j = 0; i < N && j < batchSize;) {
                        ApplicationInfo appInfo = new ApplicationInfo(apps.get(i), user, mIconCache, mLabelCache);

                        if (DEBUG_LOADERS) {
                            Log.d(TAG, "loadNotInDesktopAllAppsByBatch"
                                    + ", package name: " + appInfo.getPackageName()
                                    + ", class name: " + appInfo.componentName.getClassName());
                        }
                        ...
                        if (!mAlreadyInDesktopApp.isExistedShortcutFlag(
                            ...
                            if (DEBUG_LOADERS) {
                                Log.d(TAG, "loadNotInDesktopAllAppsByBatch alreay in desktop"
                                        + ", package name: " + appInfo.getPackageName()
                                        + ", class name: " + appInfo.componentName.getClassName());
                            }
                       }
                    ...
                    /*if (added.size() > 0 && blankScreens.size() > 0) {
                        for (int n = 0; n < added.size(); n++) {
                            ...
                            shortcuts.add(sInfo);
                            if (!sBgWorkspaceItems.contains(sInfo)) {
                    ...
                    if (added.size() > 0) {
                        ...
                        int curScreen = 0;
                        final ArrayList<ItemInfo> tmpAppItems = new ArrayList<ItemInfo>();
                        tmpAppItems.addAll(sBgWorkspaceItems);
                        tmpAppItems.addAll(sBgAppWidgets);

                        for (int n = 0; n < added.size(); n++) {
                            found = false;
                            for (int index = 0; index < (2 * Launcher.SCREEN_COUNT) + 1 && !found; ++index) {
                                curScreen = screen + (int) ((index / 2f) + 0.5f)
                                        * ((index % 2 == 1) ? 1 : -1);
                                if (0 <= curScreen && curScreen < Launcher.SCREEN_COUNT) {
                                    found = findEmptyCell(mApp.getApplicationContext(), tmpAppItems, tmpCoordinates, curScreen);
                                    if (found) {
                                        break;
                                    }
                                }
                            ...
                            if (!found) {
                                Log.i(TAG, "loadNotInDesktopAllAppsByBatch not find empty cell, application info: " + added.get(n));
                                break;
                            }
                        ...
                        if (!found) {
                            Toast.makeText(
                                    ...
                                    mApp.getString(R.string.completely_out_of_space),
        ...
        private List<Integer> getBlankScreens() {
            List<Integer> resList = new ArrayList<Integer>();
            if (mAlreadyInDesktopApp != null) {
                List<Integer> existedScreens = mAlreadyInDesktopApp
                        .getScreens();
                int N = Integer.MAX_VALUE;
                int i = Launcher.DEFAULT_SCREEN;
                int j = Launcher.DEFAULT_SCREEN;
                while (i < N && j > -1) {
                    if (i < Launcher.SCREEN_COUNT) {
                        if (!existedScreens.contains(i)) {
                            resList.add(i);
                        }
                        i++;
                    } else {
        ...
        synchronized (app) {
            for (int m = 0; m < applicationInfos.size(); m++) {
                ...
                if (LauncherLog.DEBUG) {
                    LauncherLog.d(TAG, "processInstallShortcut: componetname: " + applicationInfo.componentName + ",intent = " + applicationInfo.intent);
                }
                final int screen = Launcher.DEFAULT_SCREEN;
                ...
                if (!found) {
                    if (LauncherLog.DEBUG) {
                        LauncherLog.d(TAG, "installShortcut: componetname: "
                        + applicationInfo.componentName
                        + "cann't find empty space, result = " + result[0]);
                    }
    ...
    private boolean installShortcut(Context context, ArrayList<ItemInfo> items,
        ...
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "installShortcut: componetname: "
                + applicationInfo.componentName
                + ",intent = " + applicationInfo.intent
                + ", screen = " + screen + ", shortcutExists = " + shortcutExists);
        }
    ...
    private boolean findEmptyCell(Context context, ArrayList<ItemInfo> items,
            int[] xy, int screen) {
        ...
        for (int i = 0; i < items.size(); ++i) {
            ...
            if (item.container == LauncherSettings.Favorites.CONTAINER_DESKTOP) {
                if (item.screen == screen) {
                    ...
                    spanX = item.spanX;
                    spanY = item.spanY;
                    for (int x = cellX; 0 <= x && x < cellX + spanX
                            && x < xCount; x++) {
                        for (int y = cellY; 0 <= y && y < cellY + spanY
                                && y < yCount; y++) {
                            occupied[x][y] = true;
                            Log.d(TAG, "findEmptyCell screen:" + screen + ", occupied[" + x + "][" + y +"]");
                        }
                    }
                }
            }
    ...
    ApplicationInfo findApplicationInfoFromAllAppList(
                ShortcutInfo shortcutInfo) {
        ApplicationInfo info = AllAppsList
                .findAllAppListApplicationInfoByShortcutInfo(
                        mBgAllAppsList.data, shortcutInfo);
        return info;
    }
