...
class AllAppsList {
    ...
    public void add(ApplicationInfo info, boolean isForce) {
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "Add application in app list: app = " + info.componentName
                    + ", title = " + info.title + ", isForce = " + isForce);
        }
    ...
    public void addPackage(Context context, String packageName, UserHandle user) {
        ...
        for (LauncherActivityInfo info : matches) {
            add(new ApplicationInfo(info, user, mIconCache, null), true);
        }
    }
    ...
    public static ApplicationInfo findAllAppListApplicationInfoByShortcutInfo(
            ArrayList<ApplicationInfo> apps, ShortcutInfo shortcutInfo) {
        ComponentName component = shortcutInfo.componentName;
        String packegeName = shortcutInfo.getPackageName();
        final int N = apps.size();
        for (int i = 0; i < N; i++) {
            final ApplicationInfo info = apps.get(i);
            if (info.componentName.equals(component)
                    && info.getPackageName().equals(packegeName)) {
                return info;
            }
        }
        return null;
