...
public class DeleteDropTarget extends ButtonDropTarget {
    ...
    private boolean isApplicationInfoShortcut(DragSource source, Object info) {
        ApplicationInfo appInfo = null;
        if (info instanceof ShortcutInfo) {
            LauncherModel model = mLauncher.getModel();
            if (model != null) {
                appInfo = model.findApplicationInfoFromAllAppList((ShortcutInfo)info);
            }
        }

        return (appInfo != null);
    }
    private boolean isAppWidget(DragSource source, Object info) {
    ...
    private ApplicationInfo getApplicationInfo(DragSource source, Object info) {
        ApplicationInfo appInfo = null;
        if (info instanceof ShortcutInfo) {
            LauncherModel model = mLauncher.getModel();
            if (model != null) {
    ...
    public void onDragStart(DragSource source, Object info, int dragAction) {
        ...
        if (isApplicationInfoShortcut(source, info)) {
            ApplicationInfo appInfo = getApplicationInfo(source, info);
            if ((appInfo != null) && ((appInfo.flags & ApplicationInfo.DOWNLOADED_FLAG) != 0)) {
                isUninstall = true;
            } else {
                isVisible = false;
            }
        } else if (isAppWidget(source, info)) {
            isVisible = false;
        }
    ...
    private void completeDrop(DragObject d) {
        ...
        } else if (isApplicationInfoShortcut(d.dragSource, item)) {
             ApplicationInfo appInfo = getApplicationInfo(d.dragSource, item);
             if ((appInfo != null) && ((appInfo.flags & ApplicationInfo.DOWNLOADED_FLAG) != 0)) {
                 mLauncher.startApplicationUninstallActivity((ShortcutInfo) item, item.user);
             } else {
                // LauncherModel.deleteItemFromDatabase(mLauncher, item);
             }         
